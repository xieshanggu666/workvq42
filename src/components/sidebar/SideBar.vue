...
// 待我确认影响 / 待我审批的发布门禁（侧边栏角标）
const gatePending = computed(() =>
  gateStore.pendingCountFor(auth.user?.id, auth.user?.role, kb.docs)
)
...
<template>
  <aside class="sidebar">
    <nav class="links">
      ...
      <div class="link" :class="{ on: route.name === 'gateCenter' }" @click="go('/gates', {})">
        🚦 发布门禁<span v-if="gatePending" class="link-badge gate-badge">{{ gatePending }}</span>
      </div>
...
.link-badge.gate-badge { background: #4338ca; }
